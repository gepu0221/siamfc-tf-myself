...
def set_convolutional_each(X, W1, b1, stride, bn_beta, bn_gamma, bn_mm, bn_mv, filtergroup=False, batchnorm=True,
                      activation=True, scope=None, reuse=False):
    # use the input scope or default to "conv"
    with tf.variable_scope(scope or 'conv', reuse=reuse):
        # sanity check    
        W1 = tf.get_variable("W1", W1.shape, trainable=False, initializer=tf.constant_initializer(W1))
        b1 = tf.get_variable("b1", b1.shape, trainable=False, initializer=tf.constant_initializer(b1))

        if filtergroup:
            X0, X1 = tf.split(X, 2, 3)
            W0, W1 = tf.split(W1, 2, 3)
            h0 = tf.nn.conv2d(X0, W0, strides=[1, stride, stride, 1], padding='VALID')
            h1 = tf.nn.conv2d(X1, W1, strides=[1, stride, stride, 1], padding='VALID')
            h = tf.concat([h0, h1], 3) + b1
        else:
            #stride指卷积的步长
            h = tf.nn.conv2d(X, W1, strides=[1, stride, stride, 1], padding='VALID') + b1
